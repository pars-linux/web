...
from chiq.captcha.helpers import *
import datetime
...
class CaptchaTextInput(MultiWidget):
    ...
    def render(self, name, value, attrs=None):
        ...
        if settings.CAPTCHA_FLITE_PATH:
            ret = '<a href="%s" title="%s">%s</a>' %( reverse('captcha-audio', kwargs=dict(key=key)), unicode('Play captcha as audio file'), ret)
...
class CaptchaField(MultiValueField):
    ...
    def clean(self, value):
        ...
        try:
            ...
            raise ValidationError('Girdiğiniz kod resimdekiyle uyuşmuyor.')
        return value
