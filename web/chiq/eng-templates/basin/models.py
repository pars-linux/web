...
class Page(models.Model):
    ...
    def get_absolute_url(self):
        return "/press/%s/%s/%d/" % (self.issue.date.strftime("%Y/%m/%d"), self.issue.publication.slug, self.number)
...
class Bulletin(models.Model):
    ...
    def get_absolute_url(self):
        return "/press/bulletin/%s/" % self.slug

    def get_printable_url(self):
        return "/press/bulletin/%s/print/" % self.slug
